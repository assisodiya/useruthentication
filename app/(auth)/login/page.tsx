...
export default function SignInOne() {
    ...
    const loginUser = async () => {
        ...
        try {
            ...
            if (data.status === 200) {
                console.log("The user signed in", data);
                // Assuming signIn is properly imported from 'next-auth/react'
                signIn("credentials", {
                    email: authstate.email,
                    password: authstate.password,
                    callbackUrl: "/", // Redirect to the homepage upon successful sign-in
                    redirect: true,
                });
            } else if (data.status === 400) {
                setError(data.errors); // Assuming setError updates your component's state
            }
        } catch (err) {
            console.error("Error during login:", err);
            // Handle error based on the specific requirements of your application
            // For instance, setting a generic error message or logging the error
        } finally {
